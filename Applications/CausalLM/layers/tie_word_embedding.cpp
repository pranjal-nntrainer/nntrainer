...
void TieWordEmbedding::calcGradient(nntrainer::RunLayerContext &context) {
  ...
  if (mode_ == mode::embedding) {
    ...
    // // CLINE SUGGESTED FIX
    // // dweight = dy^T @ in = (out_dim, seq) @ (seq, in_dim) = (out_dim, in_dim)
    // // Need to reshape tensors to handle sequence dimension
    // // dy: [batch=1, channel=1, height=1024, width=151936] -> flatten to [1024,
    // // 151936] in: [batch=1, channel=1, height=1024, width=2560] -> flatten to
    // // [1024, 2560]

    // // Get dimensions
    // const unsigned int batch = dy.batch();
    // const unsigned int seq_len = dy.height();
    // const unsigned int vocab_size = dy.width();
    // const unsigned int hidden_size = in.width();

    // // Reshape tensors to 2D
    // nntrainer::TensorDim dy_dim_2d(batch, 1, seq_len, vocab_size);
    // nntrainer::TensorDim in_dim_2d(batch, 1, seq_len, hidden_size);

    // nntrainer::Tensor dy_reshaped = dy.getSharedDataTensor(dy_dim_2d, 0);
    // nntrainer::Tensor in_reshaped = in.getSharedDataTensor(in_dim_2d, 0);

    // // Compute gradient: dy^T @ in
    // dy_reshaped.dot(in_reshaped, dweight, true, false);
    // // CLINE SUGGESTED FIX ENDS
